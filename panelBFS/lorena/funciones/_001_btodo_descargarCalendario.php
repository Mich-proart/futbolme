...
$jda=1;
	...
	$sql='DELETE FROM proxiss WHERE uso=0 AND mantener=0 AND fallo>1';
	echo $sql.'<br/>';
	mysqli_query($mysqli, $sql);

	
	 
	$sql='SELECT id,ip,uso FROM proxiss WHERE fallo<3 ORDER BY uso DESC, fallo LIMIT 8';
	...
	if (count($proxis)<5){
	    $sql='UPDATE proxiss SET uso=0, fallo=0';
	    mysqli_query($mysqli, $sql);
	...
	$fallo=1;
		...
		foreach ($proxis as $key => $value) {
			...
			$sql='UPDATE proxiss SET fallo=fallo+1  WHERE id='.$id_proxi;
			mysqli_query($mysqli, $sql); 
			unset($proxis[$key]);
			unset($proxi);
			unset($id_proxi);
		...
		if ($fallo==0){
			$sql='UPDATE proxiss SET uso=uso+1 WHERE id='.$id_proxi;
			mysqli_query($mysqli, $sql); 
			
			include('../../../panelBack/simple/'.$carpeta.'/calendarioISQUAD.php');
			
				$partidosJornada=$partidosJson;

				unset($partidosJson);

				$equipos=array();
				foreach ($partidosJornada[1] as $vE) {
				...
				foreach ($partidosJornada[2] as $vE) {
					if ($vE['equipoLocal_id']>0 && $vE['equipoVisitante_id']>0){
						$equipos[$vE['equipoLocal_id']]['id']=$vE['equipoLocal_id'];
						$equipos[$vE['equipoLocal_id']]['equipo']=$vE['local'];
						$equipos[$vE['equipoVisitante_id']]['id']=$vE['equipoVisitante_id'];
