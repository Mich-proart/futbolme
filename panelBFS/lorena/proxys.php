...
if (isset($_GET['modo'])){
	if ($_GET['modo']=='borrarTodos'){
		$sql='DELETE FROM proxis WHERE mantener=0';
...
if (isset($_GET['modo'])){
	if ($_GET['modo']=='quitar'){
		$sql='DELETE FROM proxis WHERE id='.$_GET['id'];
		mysqli_query($mysqli, $sql);
	}
}
...
if (isset($_GET['modo'])){
	if ($_GET['modo']=='activar'){
		$sql='UPDATE proxis SET fallo=0 WHERE id='.$_GET['id'];
		mysqli_query($mysqli, $sql);
	}
	if ($_GET['modo']=='mantener'){
		$sql='UPDATE proxis SET mantener=1 WHERE id='.$_GET['id'];
		mysqli_query($mysqli, $sql);
	}
}
...
if (isset($_POST['modo'])){
	if ($_POST['modo']=='cargar'){
		...
		foreach ($proxis as $key => $ip) {
			...
			$sql="SELECT id FROM proxis WHERE ip='".$ip."'";
		    $resultadoSQL = mysqli_query($mysqli, $sql);
		    $resultado = mysqli_fetch_array($resultadoSQL, MYSQLI_ASSOC);
		    if (count($resultado)==0){
				$sql='INSERT INTO proxis(ip, uso, estado) VALUES ("'.$ip.'",0,0)';
				mysqli_query($mysqli, $sql);
			}
		}
		echo 'finalizado';
...
$sql="SELECT sum(uso) usos,sum(fallo) fallos FROM proxis";
$resultadoSQL = mysqli_query($mysqli, $sql);
$r = mysqli_fetch_all($resultadoSQL, MYSQLI_ASSOC);

$sql="SELECT id,ip,uso,fallo,estado, mantener FROM proxis ORDER BY estado DESC, uso DESC, fallo";
$resultadoSQL = mysqli_query($mysqli, $sql);
$proxis = mysqli_fetch_all($resultadoSQL, MYSQLI_ASSOC);?>
<html>
<head>
	<title>Proxys simples</title>
</head>
<body style="background-color: yellow">
<div style="width: 100%; float: left;">
	<div style="overflow: auto; height: 900px; width: 500px; float: left;">
		<p><a href="index.php">Volver</a></p><hr />
		<h3><a href="proxys.php">REFRESCAR - HTTP</a></h3>
	...
	<div style="overflow: auto; height: 900px; width: 350px; float: left; text-align: center">
		<form action="proxys.php" method="post">
