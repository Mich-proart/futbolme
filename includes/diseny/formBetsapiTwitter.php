<?php $altura=$altura??400; ?>
<meta charset="utf-8">
<div style="height: <?php echo $altura?>px; overflow: scroll">
<table width='100%' border='0' cellpadding='1' cellspacing='1' bgcolor='gainsboro'>
        ...
        <?php if (isset($origen) && $origen=='crearCalendario'){
            echo "<td id='equipo-temporada-".$fila['equipo_id']."' width='100'>".($key + 1).'-Partidos='.($fila['pL'] + $fila['pV']);
            if (($fila['pL'] + $fila['pV']) == 0) {
                echo ' - <a href="#" onclick="eliminarEquipoTemporada(event, '.$temporada_id.', '.$fila['equipo_id'].')">quitar</a>';
            }
            echo '</td>';
        } ?>
        </tr>
