...
<body>
	<?php $link = conectar();?>
	<table width='800' border='0' cellpadding='1' cellspacing='1' bgcolor='orange'>
		<tr>
			<form method='get' action='fichajes.php'>
				<input type='hidden' name='modo' value='buscar_jugador'>
				<td>
					Por id: <input type='text' name='jugador_id' size='4' value='0'>
				</td>
				<td>
					Por nombre: <input type='text' name='jugador_nombre' size='15'>
				</td>
				<td>
					Por lugar de nacimiento: <input type='text' name='jugador_lugar' size='15'>
				</td>				  
				<td>
					<input type='submit' name='buscar' value='buscar'> Jugadores que sus equipos esten en alguna LIGA
				</td>
			</form>
		</tr>
	</table>
        ...
        if ('enviar_twit_equipo' == $_GET['modo']) {
            ...
            foreach ($equipos as $key => $equipo) {
                if (strlen($equipo) > 1) {
                    $mensaje ="@".$equipo."\n";
                    $mensaje.=$msj."\n";
                    $mensaje.="Puedes verlo en ".$url.$idsequipos[$key]."\n";
                    $mensaje.="Para agregar a Mifutbolme pulsa aquí ".$urlMi.$idsequipos[$key]."\n";
	...
	<table width='100%' border='0' cellpadding='1' cellspacing='1' bgcolor='gainsboro'>
        ...
        $condicion = ' WHERE tor.categoria_torneo_id<>6 AND tor.categoria_torneo_id<>14';
	...
	<div style="float:left; width:100%">
		...
		<form method='get' action='fichajes.php'>
			...
			<br />ID del Foro: <input type="text" name="foro" value="<?php echo $foro; ?>" size="2">
			ID de BWIN:<input type="text" name="apuesta" value="<?php echo $apuesta; ?>" size="2">
			<br />
		...
		<form method='get' action='fichajes.php'>
		<input type='hidden' name='modo' value='enviar_twit_grupo'>
		<input type='hidden' name='temporada_id' value='<?php echo $_GET['temporada_id']; ?>'>
		<textarea name="mensaje" cols="50" rows="3">Calendario de <?php echo $nombreTorneo; ?></textarea> <br />	
		<input type="submit" name="enviar" value="Enviar 1 twit al grupo.">
		</form>
		<?php
            ...
            if (!$resultado = mysqli_query($link, $consulta)) {
                printf("Errormessage: %s\n", mysqli_error($link));
                            ...
                            while ($fila = mysqli_fetch_assoc($resultado)) {
                                $equipos .= $fila['slug'].',';
                                $idsequipos .= $fila['id'].',';
                                $nequipos[$contador]['equipo']= $fila['nombre'];
                                $nequipos[$contador]['id']= $fila['id'];
		...
		<input type='hidden' name='temporada_id' value='<?php echo $_GET['temporada_id']; ?>'>
		<textarea name="mensaje" cols="50" rows="3"><?php echo $nombreTorneo; ?>
    Consulta el calendario 2019-20</textarea> 
		<input type="submit" name="enviar" value="Enviar 1 twit a cada equipo del grupo.">
		</form>		

		
		<form method='get' action='fichajes.php'>
		<input type='hidden' name='modo' value='enviar_twit_widgets'>
		<input type='hidden' name='equipos' value='<?php echo $equipos; ?>'>
		<input type='hidden' name='idsequipos' value='<?php echo $idsequipos; ?>'>
		<input type='hidden' name='temporada_id' value='<?php echo $_GET['temporada_id']; ?>'>
		<textarea name="mensaje" cols="50" rows="3">Widgets próximo partido, resultados y clasificaciones del </textarea> <br />
		<input type="submit" name="enviar" value="Enviar 1 twit a cada equipo del grupo.">
		</form>	
			...
			<table width='100%' border='0' cellpadding='1' cellspacing='1' bgcolor='gainsboro'>
				...
				<tr bgcolor="<?php echo $color; ?>">
					<td>
						...
						<a href="?temporada_id=<?php echo $_GET['temporada_id']; ?>&equipo_id=<?php echo $fila['id']; ?>">
							<?php echo $fila['id']; ?>
						</a>
                    ...
                    <form id="f-<?php echo $fila['id']; ?>" onsubmit="grabarTwitter(event, $(this).serialize(),<?php echo $fila['id']; ?>)">
                    <input type="hidden" name="equipo_id" value="<?php echo $fila['id']; ?>">
                    <input type="hidden" name="temporada_id" value="<?php echo $_GET['temporada_id']; ?>">
                    <td>
                        <input type="text" name="slug" size="10" value="<?php echo $fila['slug']; ?>">                    
                        <input type="text" name="debut_nacional" size="2" value="<?php echo $fila['debut_nacional']; ?>">
                        <input type="submit" name="grabar" value="@">
                    </td>
                    </form>

					<td><div id="alerta-<?php echo $fila['id']; ?>"></div><?php echo $fila['nombre']; ?> (<?php 
                        if ($carp=='00pnfg'){ ?>
                        <a href="<?php echo $urlClub.$fila['futbolbase']?>">Fed</a>
                        <?php } ?>) <a href="/panelBack/club/club.php?id=<?php echo $fila['club_id']; ?>" target="_blank">Edit</a></td>
                ...
                <form method='post' action='fichajesBuscarTw.php'>
