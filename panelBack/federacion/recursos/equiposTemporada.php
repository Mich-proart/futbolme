...
if (@file_exists($f)) { 
	...
	if ($_GET['comunidad_id']==2){
		$fed=str_replace('competiciones_publico_equipo.php?id_equipo=', '', $value['url']);
		$fed=explode('&', $fed);
		$fed_id=$fed[0];
	} else {
	...
	$sq='SELECT id,nombre,codigoRFEF FROM equipo WHERE federacion_id='.$fed_id;
	$resultadoSQL = mysqli_query($mysqli, $sq);
    $resultado = mysqli_fetch_all($resultadoSQL, MYSQLI_ASSOC);
	?>
		<tr><td><?php echo $value['equipo']?></td>
			<td><?php echo $fed_id?></td>
			<td>

				
				<?php foreach ($resultado as $k => $v) { ?>
				<input type="checkbox" name="fed_id[]" value="<?php echo $v['id']?>">
				<?php echo $v['nombre'].' '.$v['id'].' '.$v['codigoRFEF'].'<br />';
				} ?>
				
			</td>
		</tr>
	<?php } ?>
	<tr><td colspan="5" align="right"><input type="submit" name="agregar" value="agregar">
	</td>	
	</table>
	</form>
<?php } else {
