...
if (isset($_GET['partido_id'])) {
    $mysqli = conectar();
    $sql = 'DELETE FROM partido WHERE id='.$_GET['partido_id'];
...
foreach ($partidos as $fila) {
    ...
    echo '<tr bgcolor="'.$colorFila.'">
      ...
      <td width="520">'.$fila['temporada_nombre'].'</td>
      <td width="60"><a href="?partido_id='.$fila['id'].'">Q</a></td>      
      <td width="100" align="center"><a href="/temporada.php?id='.$fila['temporada_id'].'" target="_blank">'.$fila['jornada'].'</a> '.$fed.'</td>
     <td>'; ?>
...
<table bgcolor="gainsboro"> 
    ...
    <?php if ($fila['apiRFEFgrupo']>0){
        $sq='SELECT apifutbol, apiRFEFcompeticion, apiRFEFgrupo, comunidad_id FROM torneo WHERE id=(SELECT torneo_id FROM temporada WHERE id='.$fila['temporada_id'].')';
        $mysqli = conectar();
