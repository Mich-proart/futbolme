...
$co=$_POST['co'];
    ...
    foreach ($proxis as $key => $value) {
        ...
        imp($id_proxi);
        imp($proxi);
        $content=getPagePlus($url,$proxi,$id_proxi);
         
        if (strlen($content)>5000) { break; }
    ...
    echo '<br />fallo='.$fallo.' bd utilizada...'.$bd.' proxi: <b>'.$proxi.'</b> <a href="?bd='.$bd.'&id_proxi='.$id_proxi.'">Eliminar proxi</a>-- <br />url: '.$url;
    $partidosJson=array();
    $tipo_torneo=1;
    ...
    if ($fallo==0){
        ...
        switch ($comunidad_id) {
            ...
            case 16:
            case 17:                  
                include('../../simple/'.$carpeta.'/extraerDatosPartidos.php');
                break;
            case 5:
                include('../../simple/'.$carpeta.'/edpCAT.php');
                break;
            case 1:
            case 7:
            case 9:
            ...
            case 11:
            case 18:  
                include('../../simple/'.$carpeta.'/extraerDatosPartidosM.php');
                break;
            case 0:
                include('../../simple/'.$carpeta.'/edpNac2.php');
                break;
        }
    ...
    if (count($partidosJson)>0) {
        ...
        //imp($partidosJson);
            ...
            unset($valorMasdatos);
            unset($goles_local1);
            unset($goles_visitante1);
            unset($gl1);
            unset($gv1);            
       
            $file = '../../json/actualizarFed/'.$temporada_id.'_'.$jornadaActiva.'_federacion.json';
            echo ' --- <a href="'.$file.'" target="_blank">Federacion</a><hr />';
            
            guardarJSON($partidosJson, $file); 
           
            

            echo 'partidosFed OK <br />';
    } else { echo 'sin resultados fed <br />'; }
