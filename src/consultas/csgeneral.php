...
function XsacarBandera($equipo_id)
{
    ...
    $mysqli = conectar();
    $resultadoSQL = mysqli_query($mysqli, $consulta);
    $bandera = mysqli_fetch_all($resultadoSQL, MYSQLI_BOTH);

    $resultado = $bandera[0][0];
...
function XtipoFederacion($temporada_id,$fase,$c,$g,$j,$co,$equipos){
    ...
    require_once ('./simple/urlFederaciones.php');
    
    require_once ('./simple/actualizadorConex.php');    
    if ($carpeta=='00pnfg'){
    $urlClasi.='codjornada='.$jornadaActiva.'&codcompeticion='.$competicion_id.'&codgrupo='.$grupo_id.'&codjornada='.$jornadaActiva;
    }
    if ($carpeta=='00isquad'){
    $urlClasi.='id_competicion='.$competicion_id.'&id_fase='.$fase.'&id_grupo='.$grupo_id.'&jornada='.$jornadaActiva;
    }
    $mysqliFB = conectarFB(); 
    $sql='DELETE FROM '.$bd.' WHERE fallo>1 AND uso=0 AND mantener=0';
    $resultadoSQL = mysqli_query($mysqliFB, $sql);

    $sql='SELECT id, ip, uso FROM '.$bd.' WHERE fallo<6 ORDER BY uso DESC, fallo LIMIT 5';
    $resultadoSQL = mysqli_query($mysqliFB, $sql);
    //echo $sql;
    ...
    foreach ($proxis as $key => $value) {
        ...
        $content=getPageEspecial($urlClasi,$proxi,$id_proxi);
        ...
        //echo ' - Quedan '.$totalProxis.' proxis por usar --> '.$content.'<br />';
    ...
    if ($fallo==0){
        ...
        switch ($comunidad_id) {
            ...
            case 11:
            case 18:  
                include('./simple/'.$carpeta.'/extraerClasificacion.php');
                break;
            case 0:
    ...
    $ruta = './json/temporada/'.$temporada_id;            
       
    guardarJSON($partido, $ruta.'/partidos.json');
    //echo $ruta.'/partidos.json grabado correctamente....<br />';
    guardarJSON($clasificacion, $ruta.'/clasFederacion.json');
    //echo $ruta.'/clasFederacion.json grabado correctamente....<br />';
