...
function prepararMenu($array)
{
    ...
    foreach ($array as $item) {
        ...
        if (1 == $item['categoria_torneo_id'] and 1 == $item['tipo_torneo'] and 1 < $item['imagenComunidad']) {
            $menu['tercera'][] = $item;
...
//*************************************************FUNCIONES TWITTER****************
function capturaTwit($equipo_id,$twEquipo,$cachetime){    
    $f = './json/twits/'.$equipo_id.'_twits.json';     
    $t=array(); // si existe y el tiempo de hoy menos la antiguedad del fichero es menor que cachetime
    if (@file_exists($f) && (time() - filemtime($f)) < $cachetime) {
        $json = file_get_contents($f);
        $t = json_decode($json, true); //leemos el fichero
    } else {
        $t=returnTweet($twEquipo, $equipo_id); //generamos el fichero
        if (!isset($t['errors']) ) { //si no hay errores guardamos el json.        
...
function returnTweet($usuario, $equipo_id, $origen=0)
{
    ...
    $consumer_key='4wzNd8fyaskkfNm4ewX72KG2Q';
    $consumer_secret='p8tuPLO0kyyqvZOzoH5D2HE5CBEAWyLIeob5iRv9qxBxWV8MwY';
    $oauth_access_token='1298181681229312001-R2VHc5G2VQIFF6TBC59iKWAU3Nanuq';
    $oauth_access_token_secret='OExyAXoDgEyFdSzipbeCSWs8Gha6O8uDy4QeloYkGQtq3';    
...
function convertirUrls($ret)
{
    ...
    $ret = preg_replace("/<a href=(.*?)>(.*?)<\/a>/", '<a  rel="nofollow" href=$1>Ver en twitter</a>', $ret);
...
function prepararGruposEliminatorio($temporada_id, $fases)
{
    ...
    foreach ($fases as $fase) {
        if (3 == $fase['tipo_eliminatoria']) {
            $gruposFaseSQL = Xfases_grupos($temporada_id, $fase['fase_id']);
...
function prepararTelevisados($array)
{
    $televisados = array();
...
function getPagePlus ($url,$proxy,$id) {
...
function getPageEspecial ($url,$proxy,$id) {
...
"Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko; compatible; Googlebot/2.1; +http://www.google.com/bot.html)" 
    ...
    $useragent = "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; .NET CLR 2.0.50727; InfoPath.1; MS-RTC LM 8)";
    imp($useragent);
    $timeout= 5;
    $dir            = dirname(__FILE__);
    $cookie_file    = $dir . '/cookies/' . md5($_SERVER['REMOTE_ADDR']) . '.txt';
...
function quitarFuncionEspecial($gol) {
