...
$diaMenos7 = new \DateTime();
$diaMenos7 = $diaMenos7->modify('-7 day')->format('Y-m-d');
echo 'Siete días: '.$diaMenos7.'<br />';
...
WHERE p.fecha<="'.$hoy.'" AND tor.tipo_torneo=1  AND p.estado_partido=0 AND tor.comunidad_id='.$_GET['comunidad_id'].' AND p.equipoVisitante_id>0 AND p.equipoLocal_id>0 GROUP BY p.temporada_id, p.jornada'; 
...
if ($modo==0){
    ...
    echo '<a href="asaltoComunidadPendientes.php?modo=1&comunidad_id='.$_GET['comunidad_id'].'">Asalto</a>'; die;
}
