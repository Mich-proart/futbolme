...
if (isset($_GET['torneo_id'])){
    $consulta = 'SELECT p.temporada_id, p.jornada, t.torneo_id, tor.tipo_torneo, tor.comunidad_id, tor.nombre, tor.apifutbol, tor.apiRFEFcompeticion, tor.apiRFEFgrupo, tor.estado, tor.orden
FROM partido p 
...
WHERE p.fecha<"'.$dia7.'" AND p.fecha>NOW() AND tor.tipo_torneo=1 AND tor.categoria_torneo_id=17 AND tor.estado<100 AND p.estado_partido<>1 AND p.equipoVisitante_id>0 AND p.equipoLocal_id>0 GROUP BY p.temporada_id'; 
}
...
if ($modo==0){
    ...
    foreach ($torneos as $kt => $torneo) {
        ...
        $comunidad_id=$torneo['orden']; 
        $comunidad_id=substr($comunidad_id, 3,2);
    ...
    echo '<a href="asaltoSala.php?modo=1">Asalto</a>'; die;
}
