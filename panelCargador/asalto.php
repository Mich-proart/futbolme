...
if ($horaSis==1){
    $consulta='UPDATE torneo SET estado=0';
...
if (isset($_GET['torneo_id'])){
    $consulta = 'SELECT p.temporada_id, p.jornada, t.torneo_id, tor.tipo_torneo, tor.comunidad_id, tor.nombre, tor.apifutbol, tor.apiRFEFcompeticion, tor.apiRFEFgrupo, tor.estado, tor.categoria_torneo_id, l.jornadas
FROM partido p 
...
INNER JOIN torneo tor ON tor.id=t.torneo_id
INNER JOIN liga l ON tor.id=l.id
WHERE p.fecha<"'.$dia7.'" AND p.fecha>NOW() AND tor.tipo_torneo=1 AND tor.comunidad_id>1 AND comunidad_id<>5 AND comunidad_id<>13 AND tor.estado<>100 AND p.estado_partido<>1 AND p.equipoVisitante_id>0 AND p.equipoLocal_id>0 GROUP BY p.temporada_id'; 
}
...
if (isset($_GET['fecha'])) {
    ...
    $consulta = 'SELECT p.temporada_id, p.jornada, t.torneo_id, tor.tipo_torneo, tor.comunidad_id, tor.nombre, tor.apifutbol, tor.apiRFEFcompeticion, tor.apiRFEFgrupo, tor.estado, tor.categoria_torneo_id, l.jornadas, 
    ec.nombre localCorto, ef.nombre visitanteCorto, p.fecha, p.hora_prevista
    FROM partido p 
    ...
    INNER JOIN torneo tor ON tor.id=t.torneo_id
    INNER JOIN liga l ON tor.id=l.id
    WHERE p.fecha="'.$_GET['fecha'].'" AND tor.visible>69 AND p.hora_prevista="00:00:11" AND p.estado_partido=0 
...
if ($modo==0){
    ...
    echo ' - <a href="?p=1">Poner los torneos estado=0</a><br />';
    ...
    foreach ($torneos as $kt => $torneo) {
        ...
        $jornadasTorneo=$torneo['jornadas'];
        ...
        $textoP='';$textoC='';
        echo $nombreTorneo.' Temporada id:'.$temporada_id.' - jornada:'.$jornadaActiva.' - Torneo_id: '.$torneo_id.' - Grupo id:'.$grupo_id; 
        include ('../panelBack/federacion/funciones/urlFederaciones.php');

        echo ' - Base de datos: <b>'.$bd.'</b> - <a href="/panelBack/federacion/?comunidad_id='.$comunidad_id.'" target="_blank">Comunidad_id: '.$comunidad_id.'</a><br />';
        ...
        echo '<a href="'.$url.'" target="_blank">Fed</a> - <a href="/panelBack/crearCalendario.php?temporada_id='.$temporada_id.'&categoria_torneo='.$categoria_torneo_id.'&tipo_torneo=1" target="_blank">Edit</a> <hr />';
    }

    
    if (isset($_GET['fecha'])){
        echo '<hr /><a href="asalto.php?modo=1&fecha='.$_GET['fecha'].'">Asalto</a>';
    } else {
        echo '<hr /><a href="asalto.php?modo=1">Asalto</a>';  
    }
