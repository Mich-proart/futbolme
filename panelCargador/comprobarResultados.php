...
if (isset($_GET['torneo_id'])){
    $modo=1;
    $consulta = 'SELECT p.temporada_id, p.jornada, t.torneo_id, tor.tipo_torneo, tor.comunidad_id, tor.nombre, tor.apifutbol, tor.apiRFEFcompeticion, tor.apiRFEFgrupo, tor.estado, tor.categoria_torneo_id FROM partido p 
INNER JOIN temporada t ON t.id=p.temporada_id
INNER JOIN torneo tor ON tor.id=t.torneo_id
WHERE p.fecha<="'.$hoy.'" AND tor.id='.$_GET['torneo_id'].' AND p.equipoVisitante_id>0 AND p.equipoLocal_id>0 AND p.tipo_partido<>1 GROUP BY p.temporada_id, p.jornada'; 

 } else {
    $consulta = 'SELECT p.temporada_id, p.jornada, t.torneo_id, tor.tipo_torneo, tor.comunidad_id, tor.nombre, tor.apifutbol, tor.apiRFEFcompeticion, tor.apiRFEFgrupo, tor.estado, tor.categoria_torneo_id FROM partido p 
INNER JOIN temporada t ON t.id=p.temporada_id
INNER JOIN torneo tor ON tor.id=t.torneo_id
WHERE p.fecha<="'.$hoy.'" AND tor.tipo_torneo=1  AND tor.estado<>300 AND tor.comunidad_id='.$_GET['comunidad_id'].' AND p.equipoVisitante_id>0 AND p.equipoLocal_id>0 AND p.tipo_partido<>1 GROUP BY p.temporada_id, p.jornada'; 
}   
...
<table><tr><td valign="top" width="85%"> 
    ...
    foreach ($torneos as $kt => $torneo) {
        ...
        $textoP='';$textoC='';
        echo $nombreTorneo.' <a href="/temporada.php?id='.$temporada_id.'&jornada='.$jornadaActiva.'" target="_blank">Temporada id:'.$temporada_id.'</a> - Torneo_id: '.$torneo_id.' - Grupo id:'.$grupo_id; 
...
<td valign="top" width="15%">
    <?php echo '<a href="comprobarResultados.php?modo=1&comunidad_id='.$_GET['comunidad_id'].'">comprobar</a>'; ?>
</td></tr></table>
<?php 
...
echo $nombreTorneo.' Temporada id:'.$temporada_id.' - Torneo_id: '.$torneo_id.' - Grupo id:'.$grupo_id.'<br />'; 
    ...
    echo '<b>Por jugar: '.$porJugar.'</b><br />';
    //imp($partidosJornada);
    //imp($fechasDat);
...
$message='<h2>Comprobador - Torneos actualizados '.$torneosActualizados.' de '.count($torneos).'</h2><br />';
