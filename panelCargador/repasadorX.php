...
if ((count($torneos)-$torneosActualizados)>0) {

    echo 'Actualizando en 10 minutos....'; ?>
...
function partidosTodos($fecha)
{
    ...
    $condicion = ' WHERE p.fecha="'.$fecha.'" AND p.hora_prevista<TIME(DATE_ADD(NOW(), INTERVAL -2 HOUR)) AND p.estado_partido=0 AND tor.visible>4 AND tor.apiRFEFcompeticion>0 AND tor.apiRFEFgrupo>0 AND tor.comunidad_id<>5 AND tor.comunidad_id<>13 AND tor.comunidad_id>1';
