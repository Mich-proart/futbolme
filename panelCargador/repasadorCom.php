...
$c=$_GET['c']??0;
$c=($c+1);
if ($c<2){ echo 'El valor de $c no es válido'; die; }

$h='';
$partidos = partidosTodos($c);
...
function partidosTodos($c)
{
    ...
    $condicion = ' WHERE p.fecha="'.$fecha.'" AND tor.comunidad_id='.$c.' AND p.hora_prevista<TIME(DATE_ADD(NOW(), INTERVAL -2 HOUR)) AND p.estado_partido=0 AND tor.visible>3 AND tor.apiRFEFcompeticion>0 AND tor.apiRFEFgrupo>0 AND tor.comunidad_id<>5 AND tor.comunidad_id<>13';
    $orden = ' ORDER BY tor.estado, p.hora_prevista DESC,ct.orden, tor.orden';
