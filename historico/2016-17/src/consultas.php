...
function conectar()
{
    ...
    if (isset($_SERVER['DATABASE'])) {
       ...
       $mysqli = mysqli_connect('localhost', 'futbolme_2016', 'A3r0r3d', $bbdd);             
...
WHERE p2.equipoLocal_id=p.equipoVisitante_id AND p2.equipoVisitante_id=p.equipoLocal_id AND p2.temporada_id=p.temporada_id AND p2.estado_partido=1 AND p2.grupo_id IS NULL LIMIT 1) as ida ";
    ...
    if ($temporada_id<650) {
        ...
        guardarJSON($resultado, $carpeta.'/partidos.json');
        //echo "Creado el fichero ".$carpeta.'/partidos.json<br />';    
    }
...
p.goles_local,
p.goles_visitante,
ec.nombre local, ec.nombreCorto localCorto, ec.foro foro_local, ec.slug twitter_local, ec.id local_id,
p.equipoLocal_id,
ef.nombre visitante, ef.nombreCorto visitanteCorto, ef.foro foro_visitante, ef.slug twitter_visitante, ef.id visitante_id,
p.equipoVisitante_id,
p.comentarios, p.observaciones, p.youtube_id,
...
tor.tipo_torneo,
tor.comunidad_id,
tor.apuesta apuesta_torneo, tor.desarrollo,
...
p.acta, e.id estadio_id, e.nombre estadio_nombre, l.nombre estadio_localidad";
    ...
    $union .= ' INNER JOIN fase fa ON p.jornada=fa.id';
    $union .= ' LEFT JOIN arbitro a ON a.id=p.arbitro_id';
...
function XequipoClub($equipo_id)
{
    ...
    e.id_original, es.id imagenEstadio, es.inaguracion, es.capacidad, es.direccion direccionEstadio, e.foro, c.es_seleccion, cat.nombre categoriaN';
    $tabla = ' equipo e';
