...
{
    $resultado = menu(); // en src/consultas/consultaPortada
...
function menu()
{
    $campos = 'tor.id, tor.nombre, tor.categoria_torneo_id, tor.tipo_torneo, te.id temporada_id,
    ...
    $condicion = ' WHERE tor.visible>4 AND tor.id>7';
...
AND ef.nombre<>'SIN PAIS' AND tor.visible>4";
...
function returnTweet($usuario, $equipo_id)
{
...
function prepararFasesEliminatorio($array)
{
    ...
    foreach ($array as $item) {
        $fases[$item['eliminatoria_id']] = $item;
    }
...
function prepararGruposEliminatorio($temporada_id, $fases)
{
    ...
    foreach ($fases as $fase) {
        if (3 == $fase['tipo_eliminatoria']) {
            $gruposFaseSQL = Xfases_grupos($temporada_id, $fase['eliminatoria_id']);
            ...
            if (count($gruposFase) > 0) {
                $grupos[$fase['eliminatoria_id']] = $gruposFase;
            }
        }
    }
...
function nombreDiaCompleto($fecha)
{
    ...
    $fecha = gmmktime(0, 0, 0, date('n', $fecha), date('j', $fecha), date('Y', $fecha));
    setlocale(LC_TIME, 'spanish');
    $nombre = strftime('%A, %d de %B de %Y', $fecha);
