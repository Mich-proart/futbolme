...
//echo $consulta;
	...
	e.desaparecido, c.presidente, c.socios, c.patrocinador, c.presupuesto, c.web, es.id imagenEstadio, es.inaguracion, es.capacidad, es.direccion direccionEstadio, c.es_seleccion, cat.nombre categoriaN';
        ...
        $resultado1 = $this->db->fetch($resultadoSQL);

        $sql = 'SELECT te.temporada_id, tor.tipo_torneo tipo, tor.visible, tor.nombre, te.grupo, tor.categoria_torneo_id, tor.division_id FROM temporada_equipo te
	...
	WHERE te.equipo_id=' . $equipo_id . ' AND tor.visible>3 AND te.temporada_id<>442';
        ...
        $resultado2 = $this->db->fetchAll($resultadoSQL);
    ...
    {
        $campos = 'e.id, e.club_id, e.nombre equipo, e.categoria_id, c.nombre categoria, e.desaparecido, e.slug, (select temporada_id from temporada_equipo where equipo_id=e.id and grupo=1) torneo';
        $tabla = ' equipo e';
...
//        $partidos = array();
    ...
    function XequipoGoles($equipo_id)
    {
        ...
        $tiempo_inicio = $funcionesHelper->microtime_float();
        ...
        $tiempo_fin = $funcionesHelper->microtime_float();
    ...
    {
        $campos = 'j.id, j.nombre, j.apodo, j.es_fichaje, j.es_baja, j.posicion, j.imagen, j.dorsal,  
    CASE posicion
    WHEN 1 THEN 0
...
///PARA ZONA USUARIO
    ...
    {
        $consulta = 'SELECT te.temporada_id,te.equipo_id,tor.nombre,tor.tipo_torneo, li.jornadaActiva valorJornada, li.jornadas
