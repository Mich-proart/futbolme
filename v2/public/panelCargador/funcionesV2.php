...
function torneosApiBet()
{
    ...
    AND tor.betsapi>0 AND tor.pais_id=60';   
...
function torneosApiBeteu1()
{
    ...
    $condicion = ' WHERE p.fecha="'.$dia.'" 
    AND p.estado_partido<>1 
    AND tor.betsapi>0 AND tor.pais_id<>60 AND tor.id<400';   
    $consulta = 'SELECT '.$campos.' FROM '.$tabla.$union.$condicion;
    //echo $consulta."<br />";
    $mysqli = conectar();
    $resultadoSQL = mysqli_query($mysqli, $consulta);
...
function torneosApiBeteu2()
{
    ...
    $condicion = ' WHERE p.fecha="'.$dia.'" 
    AND p.estado_partido<>1 
    AND tor.betsapi>0 AND tor.pais_id<>60 AND tor.id>399';   
    $consulta = 'SELECT '.$campos.' FROM '.$tabla.$union.$condicion;
    //echo $consulta."<br />";
    $mysqli = conectar();
    $resultadoSQL = mysqli_query($mysqli, $consulta);
...
function vaciarMovimientos()
{
    ...
    //if (1 == $modificaciones) { generarPartidosDia(); } se quita para optimizar
