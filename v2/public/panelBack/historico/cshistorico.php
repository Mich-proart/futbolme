...
function guardarDatos($temporada){
    ...
    foreach ($resultado as $key => $value) {
    	...
    	$sq2='SELECT jornada, fecha, (select nombre from equipo where id=equipoLocal_id) casa, (select nombre from equipo where id=equipoVisitante_id) fuera, observaciones,  goles_local, goles_visitante, hora_prevista, equipoLocal_id, equipoVisitante_id, estado_partido FROM partido WHERE temporada_id='.$temporada_id.' ORDER BY jornada, fecha, hora_prevista';

        $resultadoSQL = mysqli_query($mysqli2, $sq2);
        $partidos = mysqli_fetch_all($resultadoSQL, MYSQLI_ASSOC);

        foreach ($partidos as $k => $v) {
            $sql='INSERT INTO nacionalcalendario(jornada, fecha, nomCasa, nomFuera, cosas, resulCasa, resulFuera, hora, idTemporada, fm_local_id, fm_visitante_id, fm_arbitro_id) VALUES ("'.$v['jornada'].'", "'.$v['fecha'].'", "'.$v['casa'].'", "'.$v['fuera'].'", "'.$v['observaciones'].'", "'.$v['goles_local'].'", "'.$v['goles_visitante'].'", "'.$v['hora_prevista'].'", "'.$idTemporada.'", "'.$v['equipoLocal_id'].'", "'.$v['equipoVisitante_id'].'", "'.$v['estado_partido'].'")';
