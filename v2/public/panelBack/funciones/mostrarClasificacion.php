...
?>
<div id="muestras-error"></div>	
<table class="table" bgcolor="gainsboro">
	<thead>
		<tr>
			<th colspan="10">
	...
	<tbody>
        ...
        foreach ($clasificacion['clasificacionFinal'] as $fila) {
			...
			<tr bgcolor="white">
				...
				<td onclick="mostrarSancion(<?php echo $fila['equipo_id']?>,<?php echo $temporada_id?>,<?php echo $tipo_torneo?>)">						
            ...
            $ascensos = mysqli_fetch_all($resSQL, MYSQLI_ASSOC);
            foreach ($ascensos as $ascenso) {
                echo $ascenso['nombre'].' - Pos. <b>'.$ascenso['posicion']."</b>";?>
                <span style="cursor:pointer" 
					onclick="dieAscenso(<?php echo $ascenso['id']?>,<?php echo $tipo_torneo?>,<?php echo $temporada_id?>,<?php echo $jornadaActiva?>)"><b>Q</b></span>
                <?php echo "<br />";
            } ?>
			...
			<tr id="equipo<?php echo $fila['equipo_id']; ?>" style="display:none">
				<td colspan="10">
				<div id="sancion<?php echo $fila['equipo_id']?>">
				<form onsubmit="addSancion(event,$(this).serialize(),<?php echo $tipo_torneo?>,<?php echo $temporada_id?>,<?php echo $jornadaActiva?>)" method="post">
				...
				<form onsubmit="addAscenso(event,$(this).serialize(),<?php echo $tipo_torneo?>,<?php echo $temporada_id?>,<?php echo $jornadaActiva?>)" method="post">
				<input type="hidden" name="temporada_id" value="<?php echo $temporada_id; ?>">
				<input type="hidden" name="equipo_id" value="<?php echo $fila['equipo_id']; ?>">
			...
			<tr id="posicion<?php echo $i; ?>" style="display:none">
                ...
                ?>
					...
					<button><a href="/panelBack/funciones/clasificacion/index.php?torneo_id=<?php echo $torneo_id?>&posicion=<?php echo $i?>&grupo=<?php echo $grupo?>" target="blank" style="color:black;background-color:white;">A</a></button>
	...
	<tr><td>
                ...
                foreach ($sanciones as $sancion) {
					...
					<span style="cursor:pointer" 
					onclick="dieSancion(<?php echo $sancion['id']?>,<?php echo $tipo_torneo?>,<?php echo $temporada_id?>,<?php echo $jornadaActiva?>)"><b>Q</b></span>
                <br />
              <?php }	?>
	</td></tr>
	</table>
