...
$cadena = $_POST['cadena']??'AthleticClub';

echo '<h3>'.$cadena.' -- Imposible acceder a la API de twitter</h3>'; die;
...
class Twitter
{
    ...
    {
        $rawdata = [];
        $json = json_decode($jsonraw, true);//        $num_items = count($json);        
        $json = $json['statuses'];
        $contador = 0;
        foreach ($json as $key => $value) {            
            $fecha = $value['created_at'];
            $tweet = $value['text'];
            if ('RT' == substr($tweet, 0, 2)) { continue; }
    ...
    public function displayTable($rawdata)
    {
        ...
        for ($i = 1; $i < count($rawdata[0]); $i = $i + 2) {
            next($rawdata[0]);
            echo '<th><b>'.key($rawdata[0]).'</b></th>'; //titulos
            next($rawdata[0]);
        }
        for ($i = 0; $i < $filas; ++$i) {
            echo '<tr>';
