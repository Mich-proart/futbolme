...
function torneosFM($comunidad_id)
{
    ...
    (select concat_ws(" ,", jornada, fecha) FROM partido where temporada_id=te.id and estado_partido<>1 order by fecha, hora_prevista limit 1) proximo
    FROM torneo t 
...
function partidosPorCategoria($fecha, $ct)
{
    ...
    $union .= ' INNER JOIN temporada te ON te.id=p.temporada_id';
    $union .= ' INNER JOIN torneo tor ON tor.id=te.torneo_id';
    $condicion = " WHERE p.fecha='".$fecha."' AND tor.visible>4 AND tor.categoria_torneo_id=".$ct;
